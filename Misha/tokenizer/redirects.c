...
size_t start, size_t cnt)
{
	...
	if (token->fd_to > 1)
			close(token->fd_to);
	if (cnt == 1)
		token->fd_to = open(get_redir(l_n_m, start, &(token->line)), O_RDWR
		| O_CREAT | O_TRUNC, S_IREAD | S_IWRITE | S_IRGRP | S_IROTH);
	else
		token->fd_to = open(get_redir(l_n_m, start, &(token->line)),
...
size_t start)
{
	if (token->fd_from > 0)
		close(token->fd_from);
	token->fd_from = open(get_redir(l_n_m, start, &(token->line)),
	O_RDONLY);
}
